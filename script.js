...
function handleVoiceCommand(transcript) {
  ...
  for (const phrase in categories) {
    if (lowerTranscript.includes("to " + phrase)) {
  ...
  // Remove matched phrase only if found
  if (matchedCategory) {
    cleanTask = cleanTask.toLowerCase().replace("to " + Object.keys(categories).find(key => categories[key] === matchedCategory), "");
  }
...
function addTask(sectionId, text, save = true) {
  ...
  ["this-week", "next-week", "week-after-next", "shop"].forEach(cat => {
    if (cat !== sectionId) {
      const moveBtn = document.createElement('button');
      const label = cat === "this-week" ? "Home" : cat === "next-week" ? "Work" : "Urgent";
      moveBtn.innerText = `🔁 to ${label}`;
...
function rollWorkToUrgent() {
  const nextTasks = document.querySelectorAll("#next-week .task");
  const target = document.querySelector("#week-after-next .task-list");
  nextTasks.forEach(task => target.appendChild(task));
  saveTasks();
}

function rollHomeToUrgent() {
  const homeTasks = document.querySelectorAll("#this-week .task");
  const target = document.querySelector("#week-after-next .task-list");
  homeTasks.forEach(task => target.appendChild(task));
  saveTasks();
}
